...
for i in os.listdir("audio"): # loop through the audio file
    ...
    if iExt[1] == ".mp3":
        ...
        file = f"audio/conversion/{iExt[0]}.wav" # export a new converted version of audio to audio/conversion/
        ExtT = True
    elif iExt[1] == ".mp4":
        ...
        with open("outputs.txt","a", encoding="utf-8") as f:
            f.write(f"[{i}] not supported file type\n\n") # skip that wrong extension file
    ...
    if ExtT == True: 
        ...
        for audioC in make_chunks(audioA, 15000): # make audio into chunks
            ...
            with audioF as source:
                ...
                try:
                    with open("outputs.txt","a", encoding="utf-8") as f:
                        f.write(" | ".join((f"\n{chunkT}", repr(r.recognize_google(audioR, language="zh-TW(cmn-Hant-TW)")))))
